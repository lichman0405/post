...
- **自检比的是内容，不是形状** ✓：逐条与副本比 种类 / 字节 / 执行位 ✓ ——
  比"路径集合"说不出内容 ✓，把 main 的字节盖在任务的字节上照样通过 ✓；
  main 自己做过的同一个改动反倒被当成"丢了活儿"拒绝 ✓。
...
- **被换成目录的那条已跟踪路径，目录里的东西也一起留副本** ✓：
  ...
  所以对"任务把已跟踪路径换成了目录"这一种形状 ✓，要**走文件系统**把里面逐项收进副本 ✓
  （空的子目录、符号链接、fifo，git 对它们没有意见 ✓，但还原必须有意见 ✓）。
  **不另开一份文件清单**：清单是把挑选标准写死一次 ✓，它没列出来的就是看不见的 ✓。
- **`Files` 报这次搬过去的条目数** ✓，不再报"与新基线还差多少条" ✓。

### 测试与证据（第四轮为准）

`rebaseline_test.go` 21 个测试 `T9001`–`T9021` ✓，真 git 仓库、不 mock ✓。夹具里有：
...
**变异电池（`mutate-rebaseline-r4.py`）：27 个变体，27 caught / 0 missed / 0 invalid** ✓，
对照的是本修订的字节 ✓（`rebaseline.go` `sha256:9609b4cbe4b856ec…` ✓、
`rebaseline_test.go` `sha256:88280111750b35c8…` ✓）。每个变体只把一处修复改回原样 ✓
...
脚本现在把编译不过计为 INVALID ✓，上面引用的 27/0/0 是加了红基线前置条件之后重跑的 ✓。

### 第二轮 / 第三轮 / 第四轮 review 找出的
