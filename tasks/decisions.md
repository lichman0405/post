...
- **`T1212-TEST-01`** `POST_REQUIRE_E2E_DB=1 go test ./tests/e2e -count=1` → rc=0（`2026-09-23T02:57:38Z`）。
  非 `-v` 的那次只打印 `ok`，**不能证明旅程真的跑了**，所以另跑一次 `-v`：33 个旅程（其中
  `TestE2EDBGuardDecidesBothWays` 还带 4 个子用例，共 37 条 PASS）**0 skip、0 fail**——那 4 个子用例
  正是这条守卫自己的两向探针（`database_required` / `database_optional`）。
  **负对照**：把 DSN 指向不存在的端口、守卫仍开，同一命令 rc=1，逐条打印
...
## ㊺ 三条台账落账、T1212 的 CI 半边在 GitHub 上兑现、T1213 重钉证书（L1，2026-09-23）

### 1. 台账落账：三条 `not_run` 是**我实跑**的，不是抄工人的结论

细则与三次实跑的原文见 ㊹ §6 / §6.1（本次提交把 §6.1 从 §7 之后挪回 §6 之后；并订正了 T1212 那条的
计数写法：`-v` 日志里 37 是**含子用例的 PASS 行数**，旅程数是 33，0 skip / 0 fail）。

本窗新增的一件事：**T1212 的守卫在 CI 里得到独立兑现**——`migration-integration` 作业里新增的那一步
`integration: e2e journeys (tests/e2e, a database is required)` 在 `a8850f5` 上 success；同一次里
`go` 作业的单元步骤已改名为「integration and e2e packages excluded」。也就是说「那两个旅程在 CI 里真的
跑」这句话不再只有本机日志作证：守卫开着时跳过就等于失败，绿即真跑。

### 2. T1213 的第一次 collect 被拒：**7 条判据、6 条账目**——交付没错，账没配平（L0/L1）

逐字原因：`result-acceptance: status completed but only 6 acceptance entrie(s) for 7 criterion(a)
in the task package — every criterion needs an entry with evidence`。

**为什么这不是「东西做错了」**：同一次 collect 里 scope / residue / head-baseline / branch-ref / refs /
result-schema / result-tests-coverage 全 `[ok]`，交付物（报告 + 审计脚本）已经在这棵树上跑绿。
缺的是第 7 条判据**自己的一条** `acceptance` 条目——工人把「没有越界」的三样证据并进了第 6 条的正文
（「无越界写入」）。证据是对的，**位置**不对。

**处置**：`worker rework`（同一 session、`--resume`、diff 保留），理由文件把「只补这一条、其余一个字不许动、
`tests/acceptance/**` 一个字节都不许改」写死。**验证**（不信自述，自己核）：
- RESULT 的 `acceptance` 由 6 条变 **7** 条，第 7 条逐字抄判据原文、`status=passed`、证据三样齐全；
- 两个交付文件的 sha256 **与返工前逐字节相同**（`1741244…` / `b659e2e…`，我独立 `sha256sum` 核过）；
- 工作树仍是 `2 files changed, 374 insertions(+), 96 deletions(-)`。

**教训（写给下一本书）**：`acceptance_criteria` 是**逐条配平**的契约，不是「该说的都说了」的散文。
任务书里判据越多，越要在收工前自己数一遍条数——工人这次是**写法上把两件事合了一条**，
不是漏了工作。书里那句话以后要写成：「每一条判据都要有它自己的一条条目，哪怕内容与其他条重复」。

### 3. 我做的独立复核（G2 里人的那一半，不看自述）

- **有没有靠删断言过关**：`git diff` 里**被删掉的行只有 4 行**——两句注释（「做五件事」订正为「六件事」）
  与两行 `gates_failed` 的旧推导（被「先抹掉排版再数判定」的加强版替换）。**没有任何一条断言被删掉**。
- **判据改动是不是只加不减**：`gates_failed` 的加强（原来只认紧跟冒号的 `**未通过`，换个加粗写法就能
  让统计漏掉一个未过的门）+ 新加的「报告引的 ADR 必须在树里」断言，都是**只加不减**。
- **§6.2 的「沿用」理由**：抽查五条驱动路径，`git diff --stat b6c1fb1a..a8850f5 -- <路径>` **全部空输出**。
- **Gate B 那一处「驱动文件变了但被引用的断言没变」**：`git diff b6c1fb1a..a8850f5` 里 `human-governed`
  出现 **0 次**；该字串在三棵树里各出现 **1 次**；函数行号 `447 → 437`（断言本身在 `:621`）——三项都对得上。
- **一处对我自己不利的记录**：我第一次核这三项时用的是 `"$rev:tests/…"`，zsh 把 `:t` 当成修饰符，
  于是 `git show` 全部失败、我读到的是三个 **0**——**假读数**。改成 `${rev}:…` 后才是真值。
  这类「实测工具本身出错、而结果看起来像结论」的坑，本窗第二次出现（第一次是 §1 的计数写法）。

### 4. 独立复核的四条意见：**全部核实成立、全部记账、不返工**（L1）

独立复核的判定是 **approve**（1 次实跑复现了审计 exit 0、报告两行基准 == HEAD、两份表由同一次
`--emit-tables` 产出；`沿用` 各层逐层 `git diff --stat` 为空；Gate H 在 T1211 的仪器上重挣 —— 它自己
重跑总门得 17 行 `ok`、`MIN_CHECKS=17`、`not_asked=[]`、容器扫描那一行今天是 2110 而钉住那次是 2112）。
四条意见（2 minor + 2 nit）**全部落在 §6.8 更正表与报告头的说明文字上**，没有一条指向门判定、
证据或断言。我**逐条自己跑了命令**核过，四条全部成立：

1. **[minor] `:1230` §6.8「Gate H 的 H4 行」行**：把对象写成 `make a11y`（H4 讲的是 `make dev`），
   把位移写成「13 行」并把成因全归给 T1212。实测：`dev:` 在 `9b22339e` 是 `:324`、
   `115a4286` 与 `b6c1fb1a` 都是 `:334`（+10 来自 **T1209 的 `034a341`**）、`a8850f5` 是 `:337`
   （+3 才是 T1212）；`a11y:` 是 `431 → 441 → 441 → 444`，只挪了 3 行。**这一行更正的数值
   `:337` 与 `:311` 那句更窄的「三行下移」都是对的，错的只是这行的成因说明。**
2. **[minor] `:1228` §6.8「Gate B 的 B2 行」行**：「文件短了 13 行」是错的。实测
   `tests/e2e/conflict_e2e_test.go`：`115a4286` 与 `b6c1fb1a` 都是 **633** 行、`a8850f5` 是 **623** 行
   （`git diff --stat 115a4286..a8850f5 -- tests/e2e/conflict_e2e_test.go` = 10 insertions / 20 deletions，
   净 −10），而它自己更正的行号正好移了 10（447→437）——**和自己的数值自相矛盾**。13 是被删掉的
   `pgReachable` 块的块大小，不是文件长度变化。
3. **[nit] `:30` 报告头**：括号里那句「**上一版的**第 5.3 节 ⑧」指向一个**从来没有解析过的编号**
   （上一版 `f5a8a8a` 与本版的 §5.3 都止于 ⑦/⑥，⑧ 只出现在这句话自己里），且「本版把那一节重排为
   第 6 节」的说法不成立（`f5a8a8a..HEAD` 的 §5.3 只多了计数与 `allowed_scope` 两条 bullet）。
4. **[nit] `:1140` §6.2「Development System Gate」行**：把 `ci.yml` 的 `1 file changed, 20 insertions(+),
   7 deletions(-)` 全归给 T1210。实测那个区间有三个提交：`115a4286`（T1210 的 redis-cli 守卫）、
   `d98022a`（给 T1211 那条断言装的 pinned uv）、`a8850f5`（T1212 的 CI 半边）。**实质结论
   （「本轮一处都没动」）是对的**，错的只是归属；同一句里「有两处」→ 列了三条 → 又说「这三处」，
   自己的计数与列表也不一致。

**为什么不返工**：四条全是**说明文字的归属与数字**，本质结论与门判定一条都没错（`§6.8` 的
「上一版」列忠实抄的是上一版的数——`:324` 在上一版里就已经过期了，本版**没有**把它当新值用）。
返工一轮要重跑复核 + G2（≈40 分钟，本轮 G2 实测 19 分钟），而**这些字在「最终重钉」时必然要重写一次**
（L3 裁定会动 Gate G 与统计口径，那份报告本来就要再生成一版）。为四个词和两个数字付两轮管线，
是把「严格」花在了错的地方。

处置：**四条逐条记在这里（含我复核用的命令与数值），并列为「最终重钉」（L3 裁定之后那一笔）的
待办清单**——连同我自己发现的那一处（与第 1 条是同一行）。**不因为「复核说 approve」就把它们当没发生，
也不因为「发现了错」就把 approve 的判定推翻——两个方向都不许。**

### 5. 本窗第三个「仪器看起来像结论」：`grep -q` 配 `pipefail` 会**假拒绝**，而且是竞态

收尾取证时，我那条「报告在 main 上必须带修订记录」的守卫**报了假警**

```
REFUSED: the report at main has no 修订记录 section — T1213 has not been merged into main yet
```

——报告里那 4 个字明明在第 1091 行。原因不是内容，是**管道的退出码**：

```bash
set -o pipefail
git show <426KB 的报告> | grep -qE "修订记录"     # 管道状态 = 141，不是 0
```

`grep -q` **一命中就退出**，`git show` 随即吃到 SIGPIPE（141），`pipefail` 把整条管道报成 141，
于是 `if ! pipeline` 判成「没匹配」→ 拒绝。**它是竞态**：同一条命令我单独跑返回 **0**、在脚本里
返回 **141**（本窗实测两种都出现）。也就是说**它有时对、有时错，而错的时候看起来像一条结论**。

处置：**报告全文只取一次到变量，匹配在字符串上做**（`case ... in *修订记录*)`），并把
`head -n1` 一并换成 `sed -n '1p'`（`head` 也会提前关管道）。改完**两向实证**：对旧报告
（`115a4286`，实测 0 次）说「没有」、对新报告说「有」。

与 §3 那次的区别值得记下：zsh 的 `:t` 那次是**稳定地错**（每次读都是 0），这次是**有时错**——
后者更危险，因为「跑一遍是绿的」不再能当证据。**凡是我写的守卫，都要问一句：它这一次判对，
是因为世界真的是这样，还是因为这次没赶上竞态。**

### 6. T1213 的门与合并（实跑记录），`T1213-TEST-01` 按实跑落账

- **独立复核**：`approve`（4 条意见见 §4；无 blocking）。
- **G2**：`run-33b07e266ad03692-g2` —— **11 个作业全绿**。记录里的 `at` 只是起始时刻，
  实际从 `03:30:09Z` 跑到 `03:48:49Z`（≈19 分钟），与 `accept` 的 11:50:13 对得上
  ——**这一条我特意核过**：「记录存在」不等于「真的跑过」。
- **G3**：`run-33b07e266ad03692-g3` —— 作业 `mof-canonical`（`bash tests/acceptance/mof-canonical-workflow.sh`）
  `exit=0`，日志在 `.rddev/runtime/gates/T1213/output/run-33b07e266ad03692-g3/mof-canonical/00.log`。
- **合并**：PR **#360** → `main` 上是 **`3a1b5cf`**（12:03:24 合并）。
- **`T1213-TEST-01` 落账（我实跑，不抄工人的）**：在「报告自己钉住的基准 `a8850f51` + 合并后
  main 的 `tests/acceptance/**`」上跑 `bash tests/acceptance/v1-final-audit.sh` → **rc=0**，
  `AUDIT OK: … (0 unmerged, 1 not_run, 0 failed, 0 skipped, 0 passed-without-evidence)`；
  **负对照**（同样这些文件放在 HEAD=合并后 main 的树上）→ **rc=1**。证据
  `/tmp/evidence-t1213-20260923-040501/t1213-on-pin.log`（20 行、0 条 FAIL）。
  台账由此 **182 行全部 `passed`、0 `not_run`**。
- **第三样独立核对（我自己加的）**：在组成树上重跑 `--emit-tables`，把吐出的 **20 行非空行**
  拿去报告里逐字找 → **20/20 全部命中**，「生成基准行」与报告、与钉住的树三方相等。
  负对照（拿旧版报告比）→ 22 行里 **9 行对不上**，仪器能说不。

### 7. V1 的当前位置

`v1_required` 的 150 笔**全部 merged**；台账 182 行**全部有实跑证据**。剩下的只有两件事，
都不是工程欠账：

1. **五条 L3 裁定**（`L3-⓪`～`L3-④`，清单与我的建议已按大白话整理给 owner）——其中
...
## ㊻ owner 裁定五条 `L3`：「按你说的来。」——V1 的收尾范围就此定死（L3，2026-09-23）

### 1. 裁定本身

我把五条待裁定事项按大白话整理成一份清单（每条附我的建议与代价），owner 的回复是
**「按你说的来。」**——即**五条建议全部照准**，逐条即裁定。这是本轮唯一的 L3 输入，
此后不再有待裁定的 `L3`。

| 编号 | 裁定 | 落地在哪 | 动了什么代码 |
|---|---|---|---|
| `L3-⓪` | **不外发**：V1 不把平台内容发给平台之外的第三方服务 | `docs/55` 出平台规则、`docs/23` §6 | **没有**（现状已是「端口 + 确定性假件 + 零联网」） |
| `L3-①` | **不改**：搜索与 Research Answer 一律要求登录 | `docs/02` §3 Open Network 一句 | **没有**（契约本来就是登录后可用） |
| `L3-②` | **不要**：V1 不含大文件上传入口 | 契约删两条端点 + `docs/15`/`17`/`23`/`27` 四份规格同步 | **没有可删**（那两条端点从来没有实现） |
| `L3-③` | **V1 不含** MCP / Agent 工具面 | `docs/02` §3/§4、`docs/56` 行、`specs/mcp/tools.json` 加 `scope` 标记、`docs/47` 横幅 | **没有**（`cmd/mcp-server` 按设计返回 501） |
| `L3-④` | **保持现状**：attestation 只有知道编号才可达 | `docs/12` §2 一句（写明这是**产品决定，不是缺口**） | **没有** |

### 2. 为什么这一笔是 Supervisor 自己写的，而不是派任务

`CLAUDE.md` §8.1：`specs/**` 与 `docs/**` 是 **Supervisor-only**。Worker 的任务包永远不覆盖这两个
目录，所以「按裁定改规格」这件事**没有可以派的对象**；它也不是业务实现，是范围裁定落到纸面。
派工能覆盖的那一半（证书重钉）在下面第 5 节，仍然是 Worker 任务。

### 3. `L3-②` 的删除是**可被机械核对**的，不是「看着像没用的就删」

契约闸（`go run ./tests/cmd/contractgate -root .`）用 **HEAD 版契约**与**改后契约**各跑一次：

```
改前（HEAD）  contract_ops=40  unmounted=3
改后（工作树）contract_ops=38  unmounted=1
两次都相同    mounted=134 in_contract=30 catchall_resolved=6 exempted=0 undocumented=98
```

`unmounted=` 数的是「契约声明了、而 `cmd/**`+`internal/**` 里没有任何路由承接」的操作。删掉的
**正是**这两条（40→38 与 3→1 同时成立），而 `mounted`/`in_contract`/`undocumented` 三个数**一个都没变**
——也就是说没有任何一条**有实现**的路由因为这次删除失去契约依据。这是我敢删的依据；
「契约里这两条没见过谁用」只是线索，不是依据。

### 4. 校验与证据

- `scripts/validate_specs.py` → **12/12 通过**；`scripts/validate_openapi.py` → **7/7 通过**。
- `specs/SPEC_VERSION.json` 按 39 个输入重生成：`sha256:be4b814ff712b12d`（`--check` 一致）。
  **不手工编辑**（§8.1）。
- `go test -count=1 ./tests/contract/... ./cmd/api/assetshttp/...` → 绿。**加 `-count=1`**：
  Go 的测试缓存不追踪 `specs/**` 这类包外文件，改了规格它照样报 `(cached)`。
- `specs/mcp/tools.json` 只加两行（`scope` / `scope_note`），**保持原紧凑排版**：先按 JSON 回环
  （写下去之前 `json.loads(新文本) == json.loads(旧文本)`）校验，再逐行按原风格拼回。第一版
  我用 `indent=2` 重排，diff 从 2 行涨到 226 行——那不是这次裁定要改的东西，重做了。
- 契约闸的 MCP 计数**不受影响**（声明的 21 条工具与名字都没动，只加了标记）。

### 5. 下一步：最终重钉（Worker 任务），以及它必须带上的三件事

证书（`tests/acceptance/v1-final-*.md` 与其审计脚本）在**裁定之后**重生成一版，`Gate G` 按**范围
豁免**改判——豁免的书面依据就是这一笔落下的 `docs/02` §4 与 `docs/56` 那一行。任务书里必须写死：

1. **Gate G 的改判是「引规格」，不是「引自己」**：报告要指到 `docs/02` §4 的三条范围裁定与
   `docs/56` 的行，且**逐条点名**五条 `L3` 的裁定结果（报告 §0 与 `tasks/decisions.md` 的
   `L3` 集合相等这条机器检查不能靠改措辞绕过）；
2. **审计脚本里那句 `（Gate G）` 是写死的**（`v1-final-audit.sh` 的 `require_marker "Gate 层面：
   **N 条「通过」、M 条「未通过」（Gate G）**"`）：`M` 变成 0 之后这句话会读成「10 条通过、
   0 条未通过（Gate G）」——**判定变了而措辞的硬编码没跟上**，正是这份证书反复被咬的那类病。
   要改成「只在有未通过的门时才带名字」，并且**照旧证明它两边都判得动**；
3. **㊺ §4 记下的四条记实错误一并订正**（Makefile `dev:` 的归属与 +10、`conflict_e2e_test.go`
   是 −10 不是 13、报告头里那个不存在的「第 5.3 节 ⑧」、`ci.yml` 的 20+/7− 与它自己内部的
   计数不一致）——证书反正要重生成，四条不在合并前返工。

### 6. 这一笔**没有**做的事（写清楚，免得后来人以为它做过了）

- **没有为 V1.x 立账**：MCP 工具面、上传入口、出平台接线三件都还**没有任务**。它们现在是
  **范围外的设计**，不是 V1 的欠账；将来立账时各自带着前提——上传入口要同时定「未被使用的
  上传文件多久清掉」（`L3-②` 里我提过的那条），出平台要同时定「哪一级数据永不出平台」。
- **没有删 `specs/mcp/tools.json` 里的 21 条工具**，也没有删 `docs/47`：按裁定它们是 V1.x 的
  设计输入，删掉等于把一份已评审的面重新变回空白页。
- **没有动 `docs/31_MASTER_ACCEPTANCE.md`**：它定义的是「Gate 是什么」，改判发生在证书里；
  两者职责不同，改 docs/31 会让「Gate G 被豁免」看起来像是把门槛拆了，而不是对范围的一次裁定。

## ㊼ T1214 落地：证书做最后一次重钉、Gate G 按范围豁免，V1 完成声明四条全部成立（L1 执行，2026-09-23/24）

### 1. 这一笔是什么

㊻ 的五条裁定落到规格之后，证书必须再生成一版：`Gate G` 由「未通过」改判为**按范围豁免**通过，
并把 ㊺ §4 记下的四条记实错误一并订正。**这是证书的最后一次重钉**——钉在承载裁定的那棵树上，
之后不再有重钉任务。

交付物就是 `allowed_scope` 的全部两个文件：

- `tests/acceptance/v1-final-report.md`，**1696 行**，sha256
  `92cb6c19228a915787a4cdc90c225c54e84d0b7a447d6ed0af541b72671eff1f`，钉在 `91ed2d5`（第九轮）；
- `tests/acceptance/v1-final-audit.sh`，sha256
  `c6ae31813ebfacfe4680ddf41402e6f69077d9c4864ca892ffd24ccc29c7a77d`（第三轮**没有改它**）。

`tasks/**`、`specs/**`、`docs/**`、`.github/**`、`Makefile` **一个都没动**（我与独立复核各核过一次）。
本版合并后 `origin/main` 上这两个文件的哈希与交付**逐字相等**（`git cat-file -p origin/main:… | sha256sum`）。

### 2. 往返记录（都在 `.rddev/runtime/gates/T1214/`，不靠回忆）

三份 collect：`run-33e2ab2aacce0237`(11:42:59Z)、`run-c976e96ce287876a`(12:17:37Z)、
`run-d8fdf85359e871d8`(12:38:23Z)；四份 reject：`5605f2a8c5cba2f9`(11:51:46Z)、
`71a78c6342d9625c`(12:03:55Z)、`20c3f5f3357162e6`(12:21:09Z)、`02951ee0082f2597`(12:30:50Z)。
最后的返工信逐字在 `reject-run-02951ee0082f2597.json`（那次 rework 的 run 记录，与 `prompt.md` 同文）：
四处「记录与实物对不上」——§2 Gate H 第 3 段的出处、§6.0(c) 那句「/tmp 副本」、§6.0(e) 清扫二的
行号、§6.6–§6.8 的脚本行号。

状态历史（`tasks/task_status.json`）：`running→verification` 12:17:37 · `→rejected` 12:21:09 ·
`→running` 12:30:52 · `→verification` 12:38:23 · `→accepted` 13:09:15 · `→merged`
**2026-09-24T00:36:13Z**。

**第三轮的改法只动行内**：15 行被换（`diff -u` 下 **9 个 hunk**；`-U0` 下 10 块）、四处，
行数 1696 → **1696**（没增没减），脚本一字未改——上一轮已通过的结论因此继续成立，
不必重跑整棵树。**「9 个 hunk 还是 10 块」这句写准了**：复核的 nit 正是冲着这句话来的（见 §4）。

### 3. 我这边独立做的 G2（2026-09-24 重跑，两份日志可查）

在证书自己钉住的基准 `91ed2d5` 的工作树上、放入**本版交付**的 `tests/acceptance/**`（取自合并后的
`origin/main`）后跑：

```
bash tests/acceptance/v1-final-audit.sh   →  rc=0
AUDIT OK: report markers present, counts match (0 unmerged, 1 not_run, 0 failed, 0 skipped, 0 passed-without-evidence)
```

**负对照**（同样这些文件放在 HEAD=`67d60fb` 的树上）→ **rc=1**「FAIL: 报告的基准不是这棵树。」
日志 `/tmp/t1214-g2-20260924/pin.log` 与 `other.log`。

> 这一条上一个会话里跑过一次，但那些 `/tmp` 日志被清理了。**没有拿「我记得跑过」当证据**：
> 今天按同样的组成重跑，引用今天这两份日志——**能被别人当场复核的，才算证据**。

### 4. 独立复核：`approve`（0 blocking / 0 major），3 条 nit 逐条处置

`review-run-3540f463f4667848`：它**重跑**了报告里的承重结论（`--emit-tables` 的 24 行逐字对上、
四个方向的变异、契约闸的三个数、交付哈希与 RESULT 一致），不是读一遍。三条 nit：

1. **RESULT（不在 git 里、不进交付）**把第三轮差异写成「10 个 hunk」，按它自己给的 `diff -u` 实为
   **9 个**——已在 §2 与本节改正；
2. 报告 `:1383`/`:1398`/`:1456` 三处写的备份名是**更早那几轮**的（`backup.md`、`backup9.md`），
   本轮五条证明重跑用的是 `backup12.md`（RESULT 的 tests[4]–[8] 逐条记着正确的名字与还原后的
   sha256）。**判断：记下不返工**——被改的是「哪一轮的 `/tmp` 文件」这个操作细节，不是判定、
   不是数字；五条证明**确实在本轮重跑过**且复核独立复现；`/tmp` 路径本来就不可长期指望
   （可依赖的是方法：原地改、sha256 核对还原）；没有任何一条 `require_marker` 读这个字符串。
   工人已在 RESULT 的 risk 里点明这一处；
3. RESULT 把更早那一轮说成「第八轮返工」，而证书编号里 T1214 是第九轮、T1213 是第八轮——
   同样只在 RESULT 里，实质（脚本 74+/8− 是本任务自己的改动）经复核独立核对成立。

**为什么记录而不返工**：三条都不指任何门判定或证据，且第 2、3 条**根本不进交付**；第 1 条是
我自己记录的措辞。为它们再开一轮（约 45 分钟 + 一次重跑复核）而每一轮都会带出新的小瑕疵，
风险大于收益。**它们没被当成没发生**：都写在这里，位置与理由都可查。

### 5. 门与合并（实跑记录）

- **G2** `run-9a615a374b410be6-g2`：CI 的**全部 11 个作业**在组成树上跑，**11/11 passed**，
  实际从 `12:49:09.920Z` 跑到 `13:08:34.766Z`（≈19.5 分钟）——**这一条我特意核过**：
  「记录存在」不等于「真的跑过」；
- **G3** `run-9a615a374b410be6-g3`：作业 `mof-canonical`（`bash tests/acceptance/mof-canonical-workflow.sh`）
  `exit=0`，`13:08:36.415Z→13:09:15.334Z`；
- **accept** `run-9a615a374b410be6`：`G1/G2/G3/G4` 四门全 `passed`；
- **提交** `9596c349`（分支 `task/T1214-l3-v1-gate-g`）→ **PR #361** → `main` = **`8f393bb`**
  （`2026-09-24T00:36:11Z` 合并，合并记录 `run-3fc5d70ab80d8066`）；
- **main HEAD 的 CI**：run `35939158834` —— **11 个作业全 `success`**（`08:48:34` 收尾，
  即那条 run 在合并提交上真的跑完过，不是「排队中」被当成了绿）。

> 合并这一步是**我自己走的**：上一个会话结束时驱动在「等合并」上停住（`rddev status` →
> `dead (heartbeat stale)`，pid 1000505），PR 整夜停在 `accepted`；今早 12 项检查已全绿、
> `MERGEABLE/CLEAN`，按 §5.1 的六个条件逐条成立后用 `rddev pr merge T1214` 走完。**没有手动去点
> GitHub 的合并按钮**，也没有和驱动抢着跑 accept。

### 6. 台账落账（在合并**之后**做，这是工具定的规矩）

`T1214-TEST-01`（certificate final revision）用 `scripts/record_test_run.py` 按 §3 的实跑落账为
`passed`（`last_run: 2026-09-24T00:37:56Z`）。工具的规则是「任务没合并不许写 `passed`」，
所以账与证书的先后差是**有意的**：`91ed2d5` 那一版计数表里它是 `not_run`，报告 `:69`/`:160`
也如实这么写（「报告作者不能给自己的门记 `passed`」）。

**没有加第二行**：任务书声明了两条（第二条是 `master gate scope exemption`），但 T1210–T1213
（都已合并）都是「一笔任务一行 `task-defined`、名字取第一条」，CI 的 `TASKSTATE-TESTS-COVERAGE`
只要求每笔任务 ≥1 行；更要紧的是**报告自己写着**「183 条 = 182 `passed` + 1 `not_run`（点名
`T1214-TEST-01`）」——再加一行，这份刚合并的证书里的数字当场就假了。
第二声明的证据在这里：证书 §2 的 Gate G 一节（含 R12）+ ㊻ §1 的裁定 + `docs/02` §4 / `docs/56` 那一行。

### 7. §12 完成声明：四条全部成立

| 条件 | 判定 | 证据 |
|---|---|---|
| V1-required 全部合并 | **成立** | `v1_required=150 merged=150 not_merged=0`（合并后重测：`tasks/tasks.json` × `task_status.json`） |
| 四层 Gate（CI 等价集合） | **成立** | `main` HEAD `8f393bb`：CI run `35939158834` **11 个作业全 success**；G2/G3/G4 记录见 §5 |
| MOF canonical workflow 全闭环 | **成立** | G3 `run-9a615a374b410be6-g3`：`mof-canonical` `exit=0`；台账 `T1202-TEST-01` `passed` |
| Master Acceptance（Gate A–I） | **成立** | 证书 `92cb6c19`：**10 条「通过」、0 条「未通过」**，Gate G 按**范围豁免**通过（§2、R12） |

台账现状：183 行**全部 `passed`**、0 `not_run`、0 `failed`、0 `skipped`；
`python3 scripts/validate_task_state.py` → **9/9 通过**。

**Gate G 豁免的书面依据**（不是它自己说的）：`docs/02_V1_SCOPE.md` §4（四条带
`L3-⓪/②/③/④` 标记、写明「范围裁定，不是还没做」）、`docs/56:16`、`specs/mcp/tools.json` 的
`"scope": "post-v1"`、`docs/47` 横幅、㊻ §1。报告 §0 的五条裁定与 ㊻ 的集合相等这一条由脚本
机器检查（5 条 `**裁定**：` 行与 5 个 `L3` 标签位置配对）。

### 8. 必须留在这里的残余与边界

1. **证书是基准快照，不是常驻检查**：它钉在 `91ed2d5`，合并把 HEAD 推走之后，它的机械表说的
   仍是那棵树（报告 `:56-62` 自己写明再生条件；脚本在别的树上退出非 0 并给出两条出路）。
   **这不是缺口，是它的定义**；「最后一次重钉」的含义就是：V1 的那棵树被这一版证书钉住了。
   今天验证过：把这一版的两个文件放进 `67d60fb` 的树里 → rc=1（§3 的负对照）。
2. **落账在合并之后**（§6），所以报告里的计数表与今天的台账差一行（`T1214-TEST-01`
   `not_run` → `passed`）；要恢复一致只有在更新的树上按报告自己的办法重新生成一版。
   复核也把这一条作为 risk 记下了。
3. **Gate G 的豁免是带条件的**：它挂在 `L3-③`（V1 不含 MCP 工具面）上。若未来某版把工具面
   纳入范围，这条处置必须**退回「造出来并验过」**并附新证据——报告 §2/R12 写着触发条件，
   但树里没有任何东西强制它。
4. §6.1–§6.9 保留的是**第八轮**的叙述（路径、行号都是那一轮的原话），§6.4 的两处 T1213 路径
   是**版本对照**、故意保留；§6.0(e) 的三处备份名同属「更早那几轮的原话」（§4 第 2 条）。
5. Gate H / Gate I 的证据时刻早于本轮的行内订正——它们判的是**树**（HEAD 始终是 `91ed2d5`），
   不是报告文件；若将来有政策要求证据覆盖最终脏树，Gate H 需要重跑一次（RESULT risk 记着）。

### 9. 这一笔**没有**做的事

- **没有为 V1.x 立账**（MCP 工具面、上传入口、出平台接线仍是范围外的设计，不是 V1 的欠账）；
- **没有改 `docs/31_MASTER_ACCEPTANCE.md`**——它定义「Gate 是什么」，改判发生在证书里（照 ㊻ §6）；
...
## 61 — T1220 验收：三处我独立量过、两处评审抓到我漏了的，以及**我自己写的判据字面不可满足**（L1 记账，2026-09-24）

T1220 是第 59 条那条线索的落地：`gosec ./...` 从仓库根走文件系统，把被 gitignore 的 `.rddev/` 草稿
（rebaseline 草稿、别的工人的 worktree）当成**本仓库源码**扫，把它们的发现算成这棵树的发现。
工人改名换姓地改成「从 `go list` 的包集合派生扫描面 + 逐个文件核 root 与 gitignore」。

### 一、我独立做的 G2（不采信工人和评审的自述）

- **scope**：5 个文件，全部落在 `allowed_scope` 内；`g2-in-the-worker-tree-dirties-it` 那条规矩照办——
  跑套件是写操作，事后比对 collect 报的路径数。
- **baseline**：`ops/ci/gosec-baseline.txt` 只动注释，256 条条目逐字节相同；**0 个条目的 key 指向
  `.rddev/`**。（我先前用 `grep -c '\.rddev/'` 得到 107，一度以为文件有病；换成 `cut -f1` 看 key
  才是 0——**那 107 全在条目 reason 列的散文里**。是我的 grep 错了，不是文件错了。）
- **gosec 到底走不走文件系统**：用植入的 G404 探了两遍——`gosec ./...` 从根下去会走进子目录，
  `gosec <裸包目录>` 不会。这是这条缺陷能被做出来的前提，我自己量过才算。
- **工人那台 6 案例仪器我自己跑了一遍**（干净树绿 / 点目录副本出局 / 被忽略的非点目录被丢弃**并且报出规则**
  / 保留包内的被忽略文件按名判 FAILED SURFACE / `POST_SAST_GO_TARGET` 照扫 / 植物清除后复原）。
- **我自己的变异体**：把 `repo_paths_only … || exit 1` 改成空操作 → 案例 4 立刻红、汇总 FAIL；
  还原后 sha256 逐字节一致。**仪器能说「不」。**
- **真实环境那一对**才是决定性的：在本机主检出（带 `.rddev/`，864 个 rebaseline 草稿 + 13583 个 worktree 的 `.go`）
  改前 `FAIL sast-go: 350 finding(s), 94 unbaselined (1274 file(s) scanned)`，改后
  `ok sast-go: … 783 file(s) scanned, 256 finding(s) reported (256 baselined, 0 unbaselined)`。
- **「丢弃」这个读法凭什么对**（评审的 [1] 是判断题，我不采信它的结论，自己复算）：在**装了固定工具链**的
  T1220 树里 `go list -e -f '{{.Dir}}' ./...` 返回 **150** 个目录（没装是 149），多出来的正是
  npm 包 `flatted` 里自带的 Go 文件 `tests/security/node-tools/node_modules/flatted/golang/pkg/flatted`；
  `git check-ignore -v` 给出规则 `.gitignore:41:node_modules/`。而 `ci.yml:437` 在安全门之前跑
  `make security-tools`。**所以「拒绝被忽略的包目录」这个更严的读法会让 CI 每次都红**——
  不是更安全，是把门变成谁也无法满足的门。丢弃 + **打印路径与规则**是唯一既安全又可审计的读法。**确认。**

### 二、评审抓到的、我漏掉的两处

- **[2] `!` 否定模式被当成命中**（真缺陷，我复现了）：`git check-ignore --stdin -v` 对**被否定模式重新纳入**的
  路径也会打一行，模式字段带 `!` 前缀，而那条路径 git **并未忽略**。我自己的最小复现：
  `.gitignore` 写 `*.go` 再写 `!keep.go`，git 说 `keep.go` NOT ignored，但 `-v` 照样打出
  `inner/.gitignore:2:!keep.go` 一行 → 这一行的解析会把它读成「被忽略」，于是**误判为逃逸（假红）
  或在丢弃循环里静默缩小扫描面**。**在本仓库不可达**（唯一的否定是 `!.env.example`，不可能匹配 `.go`），
  属潜伏缺陷；修法是一行（跳过 `!` 前缀的模式字段）。→ 转为后续任务，不阻断本次合并。
- **[4] `tests/security/sast.sh:6` 的自述仍是 `# gosec over the whole Go module`**——正是本任务出现
  在标题里的那句假话的最后一处。→ 并入同一笔后续。
- 另外评审的 [3]（新仪器**没有被任何东西引用**：不在 README、不在 Makefile、不在 CI、不是 gate 行）
  与我先前的独立发现一致——工人自己也如实把它列为 risks/follow_up。

### 三、我自己写的判据**字面不可满足**

T1220 验收判据第 4 条第三分句，我写的是 `grep -c '\.rddev/' ops/ci/gosec-baseline.txt` 必须为 0。
**这是不可满足的**：HEAD 上已经是 105，全部来自 `internal/devorchestrator/**` 那些条目**原因文本里**提到
`.rddev/` 的散文，而任务书又要求条目内容不许动；新表头按任务书要求补的边界说明再加 2 行。
我是**在不知道这个前提的情况下**写下这条的。工人**如实报告了字面不达标**、并给出 key 级为 0 的实质证据——
这正是我要的行为。**判据的意图（没有任何 `.rddev/` 路径被写进 baseline 当成发现）成立；判据的字面收回。**
教训记在这里：**写判据时用「能被测量到的那个量」，而不是我脑子里那个量的字符串近似。**

### 四、裁定

**接受 T1220**（与独立评审判决一致）。[1] 我确认；[2][3][4][5] 与「15 个 `tests/contract/testdata/**`
下的 Go 夹具要不要单独一行扫描」一并转为**一笔后续任务**——但它的任务书要等 T1221 合并、
且**没有在跑的 G2** 之后再落账（改 `tasks/tasks.json` 会推动 marker，而在跑的 G2 是拿当前 main 合成的）。

### 五、教训

- **独立评审确实抓到了我漏的东西**：`!` 否定模式那一处，我读了同一份代码、跑了同一套仪器，没看出来。
  这是把这笔审查派出去的正当性，不是流程开销。
- **但评审的结论同样要复算**：「丢弃而非拒绝」这条它是当判断题交给我确认的，我没有照签——
  自己去装了工具链的树里数了 150 个目录、读了 `.gitignore:41`、核了 `ci.yml:437` 才签。
- **观察对了、解释错了这已经是第四次**（第 60 条记过一次）：这次不是机制诊断错，是**判据本身**写歪了。


## 62 — T1221 验收：五项我独立量过，以及**我自己造出来的一个发现**（L1 记账，2026-09-24）

T1221 收第 60 条的洞：一个换行绕开守卫**每一族**命令位策略。判据核心是「这个判据能红」与「188 条既有用例逐字保留」。

### 一、我独立做的（不采信工人自述）

- **scope**：8 个路径，全部在 `allowed_scope` 内。跑完我自己的探针与变异后 `git status --porcelain` 仍是**同样那 8 个**（`g2-in-the-worker-tree-dirties-it` 那条规矩照办）。
- **保留**：既有 188 条用例**逐字**保留——我自己用 `LC_ALL=C comm -23`（全文排序，不是比名字）得 0 行；新增 184 行、23 条新用例。
- **套件**：我自己跑 `guard-regression: 365/365 PASS`。floor 由 188 抬到 365 是**有意的**，`worker_guard_regression_test.go:80-89` 的注释把 365 的算术逐项列出（90 拒绝 + 66 放行邻居 + 5 线形 + 3 tab + 5 不可读载荷 + 5 错类型字段 + 1 未知工具反例 + 2 shell 读），我核过它加得起来。
- **机制**：`json_field`（文本扫描）**删除**，所有调用点改走 `json_string`；文件里只剩注释里一处提到旧名字。转义臂的不对称我复测过（字母类 `\n`→`n` 坏，`\"`、`\\` 本来就对）。
- **两侧对照**：同组载荷在**旧守卫**（main 的字节）上仍 `rc=0 allow`（换行那几行），在新守卫上全部 `rc=2 BLOCK`；`;`/`&&`/`||`/`&`/`|` 两版都拦，普通作业（两行两条普通命令）两版都放。
- **两份副本逐字节相同**：`internal/devorchestrator/embed/worker-guard.sh` 与 `.rddev.example/guard/worker-guard.sh` sha256 同为 `f353eb281defc06d…`。
- **读的那半**：工人选择**把话说准**而不是扩大强制——句子现在点名 `Read/Grep/Glob/NotebookRead`，并明说「它按命令文本判定，不管 shell 命令读了什么」。`TestTheGuardClaimInTheSystemPromptIsTrue` 现在**三条腿一起钉**：句子点名工具、`guardMatcher` 必须真的认这些工具、以及守卫在边界**两侧**的实际判决（confined 侧 3 条必拦，limit 侧 2 条必放）。我自己的变异（把 `NotebookRead` 从 `guardMatcher` 里拿掉、句子不动）→ `FAIL: …the hook is never invoked for it and the promise is prose`。**这条测试能红，且红的正是「话比机制宽」那个病。**
- **「54031 条记录调用」那句**：我自己扫了 1350 份会话转写、**137,776** 条 `tool_use`——`name`/`command`/`file_path`/`path`/`pattern`/`glob` 全是 `str`，`run_in_background` 全是 `bool`，**0 条**非字符串非 null。工人报的是子集，结论方向对且保守。

### 二、**我自己造出来的一个发现**（回放 137,832 条真实调用）

我做了这台项目里最该做的那类检查：把**每一条**有记录的 Bash 调用回放进新旧两版守卫，找出「旧放行、新拒绝」的**误拦**。为省时间我按**形状**去重（工具名 + 字段有无 + 类型 + 是否空串）——137,832 条只落成 37 种形状。结果：**2 条翻了**，都是 Bash，形状都是 `tool_input = {"__unparsedToolInput": …}`，新守卫回「carries no command string」。

看着像一个真的误拦回归（新守卫 fail-closed，确实会拒没有命令串的 Bash 调用；工人也在 tests[2] 的第 03 行**如实**记了这件事）。**但它是假的，而且假在我的量具上：**

- 那 9 处 `raw` **全部恰好 2048 字节**，且全部**在字符串中间被截断**（`json.loads` 一律 `Unterminated string`）。2048 是个**转写存储的预算**，不是线上载荷。
- 这些调用**根本没有执行**：它们的结果是 `<tool_use_error>InputValidationError: … was called with input that could not be parsed as JSON`，而错误里写着真实大小——那条 Bash 是 **119,503 字节**。**能站住的那句话是**：这 2 条调用在被守卫看到之前就已经是死调用（harness 自己拒了），所以就算守卫也拒它，**被拒的也是一条本来就执行不了的调用**——137,832 条里**没有任何一条能执行的调用被误拦**。（至于「钩子到底会不会拿到这个形状」，我**没有**证明，也不需要在这次裁定里证明：两种情形下结论相同。别把「未证明」写成「已排除」。）
- 我喂给守卫的是一份 119 KB 载荷的头 2048 字节。**是我把不存在的东西做出来再量了它。**

**教训（第三次同类，见 `prove-the-instrument-can-say-no`）**：**转写不是线路**。回放历史日志做安全判定时，「记录里的形状」与「钩子收到的形状」不是一回事；差一个截断，就能造出一条不存在的回归。判据要能红，**也要能证明红的是真的**——这一次红的是我的量具。

### 三、裁定

**接受 T1221**：[洞被堵上（两侧对照）] + [既有 188 条逐字保留] + [新判据能红（我自己的两条变异 + 评审的两条）] + [契约句子与强制绑在一处并能红] + [两份副本逐字节相同] + [137,776 条真实调用中无一被新守卫误拦]。误拦那 2 条**经查是转写截断的产物、调用从未执行**，不构成回归；工人对「无命令串的 Bash 调用现在被拒」的披露**是准确且完整的**，方向是 fail-closed 的自觉选择。

## 63 — 守卫读的「命令位」包含**写进文件的文本**：工人侧实测只有 4 条误拦，而那个「自然修法」会打开一条真的出口（L1 裁定 + 立账 T1223，2026-09-24）

T1221 把命令位分析修对了（换行是分隔符），但顺手产生了一件**没人写下来的后果**：`cat > x <<'EOF'` 的**正文**、多行 `bash -c '...'` 的**字符串内部**，也一律进命令位分析。正文里有一行以 `git push` 开头，整条调用就被拒。评审把这条记为 findings[0] **minor**，位置 `internal/devorchestrator/embed/worker-guard.sh:51`（那条「`sh -c '...'` 与同类被刻意当作不透明」的说明）。

### 一、起点：T1221 评审的四条 findings（逐字读过，不是转述）

- **[minor] `worker-guard.sh:51`**——多行**内容**被当命令位，而 Known-limits 没说；它自己的口径是「42,812 条 Bash 记录、4,211 条多行，22 条改判定，其中 16 条是基线守卫在单行写法下**本来就会拒的同一策略**，6 条纯内容」。
- **[nit] `.rddev.example/README.md:59`**——`Only an unknown tool name is allowed through` 比机制宽：`{"tool_name":"Read","tool_input":{}}` 等**已知工具但不点名路径/命令**的调用 exit 0。
- **[nit] `worker-guard.sh:621`**——`records_file=$(mktemp ...) || exit 0`，评审判为「unreachable in practice」。
- **[nit] `.rddev/workers/T1221/RESULT.json`** 一张转义表里有一行的载荷写法与其结论不自洽（`a\\u0041b` 写成单反斜杠的结果）。**这一条不派工**：它在 `.rddev/workers/` 下，**不在版本库里**，属于「记录本身的排版」而非仓库事实；作为已读到的既成事实收下。

### 二、我自己量的三件事

**1) 现象可复现，而且两侧都要看。** heredoc 正文里一行 `git push origin main`：新守卫 `rc=2`（拒），旧守卫（main 的字节）`rc=0`（放）；正文换成普通文本，两边都放。也就是说这不是「heredoc 一律拒」。

**2) 「让解析器跳过正文」不是个选项——两条机制我都跑出了副作用。** (i) 分隔符**不加引号**时，正文里的 `$(echo X > /tmp/probe)` 在消费者命令运行**之前**就被展开执行（`cat` 还没跑，`/tmp/probe` 已经落盘）；(ii) `cat <<'EOF' | sh` 把正文**喂给 shell 执行**（副作用同样落盘）。所以正文**是不是数据取决于别的行**，静态判不出来——豁免正文就是把一个安全方向的过度拦截换成一条真的出口。

**3) 发生率：我把语料按「谁在跑」拆开，因为只有工人侧的误拦是真问题。** 工人会话的目录名带 `--rddev-worktrees-TXXXX`（395 个项目目录里 351 个是工人，我自己的那个不带），据此拆开：

| 语料 | Bash 调用 | 多行 | 「点名词只出现在正文/未闭合引号里」 | 误拦 | 占多行 |
|---|---|---|---|---|---|
| **仅工人会话** | 78,138 | 8,667 | 45 | **4** | **0.046%** |
| 全部会话（含我自己的） | 116,323 | 23,281 | 178 | 30 | 0.129% |

口径要说清：**我那台量具的第一版是错的**——它只按「有 heredoc 操作符」分类，把多行 `bash -c '...'` 也算成「数据」。逐条读那 30 条才发现：**8 条是多行 `bash -c`**（正文就是要跑的脚本，里面真的有 `git worktree remove --force`、`rm -rf $PROBE`、`git worktree add`——**拒它是对的**）、**7 条是 `cat > /tmp/wait-ci*.sh <<'EOF'`** 写下脚本又在后续调用里 `bash` 它、**3 条是 `python3 - <<'PY'`**（正文就是被执行的代码），**真正「写进文件、永不执行」的只有约 3 条**（往 `gate_run_test.go`/`decisions.md` 里 `cat >>` 追加源码或散文，被注释里提到的 git 动词命中）。**所以这一类的多数是「会被执行的文本」——豁免正文放出去的比收回来的多得多。** 另有评审自己的 6/4,211（0.14%）与我这个 0.046% 同量级，**两套不同语料、同一个结论方向**。

### 三、`:621` 我把它从 nit 抬成必修

评审按**可达性**评（「要有坏掉的 TMPDIR 才碰得到」），我按**失效范围**评了一遍，结论差得远。`TMPDIR` 指到不存在的目录后：

- **被静默关掉**：`rm -f /etc/hosts`、`cp a /etc/b`、`sudo ls`、`gh pr list`、`git push origin main`、`printenv GH_TOKEN`——**全部 `rc=0` 放行**。
- **仍然生效**：`echo hi > /etc/x` 的重定向判定（`rc=2`）、`Write /etc/x` 的路径判定（`rc=2`）——这两半不走那个临时文件。

丢掉的**不是一条检查，而是 Bash 这一侧策略的绝大部分**，而它只在 stderr 留一句 `mktemp: failed to create file via template` 就继续放行。`grep -n mktemp` 数下来这是**全文唯一一处**，`TMPDIR` 也只被那一处用到，所以 `TMPDIR=/nonexistent` 能确定性地把它逼出来——**这条用例改前天然是绿的（放行），改后必须红**。附带说清：这个 `TMPDIR` 是**钩子自己的环境**，工人无法用命令前缀改它（那是子 shell，改不到父进程），所以这是**健壮性**缺陷而非现成的绕过。

### 四、裁定

- **立账 T1223**（八个位置收成一个可派工的包）：守卫头部两条说明按机制改写（`sh -c` 的不透明**只对单行成立**；写进文件的文本同样算命令位），Known limits 增一条「**为什么不豁免正文**」连同上面两条机制与那张实测构成，`:621` 改成拒，README 那句改准，四组新用例（heredoc 正文点名/良性、多行 `bash -c` 点名/良性、`TMPDIR` 建不出临时文件），并把 `worker_guard_regression_test.go` 的 `floor` 抬到落账条数。
- **不豁免 heredoc 正文**——这是 L1 决定，理由是**证明过的机制**（两条都跑出了副作用），不是偏好。这条判断必须写进注释，否则下一笔会有人「好心」把它修成一条出口。
- **`findings[2]` 不派工**（不在版本库里）。
- 允许面收在五个位置：两份逐字节同步的守卫副本、`.rddev.example/README.md`、`tests/worker-guard/**`、以及只开 `const floor` 与它那段说明的那一个 Go 测试文件（`TestWriteGuardFiles`、`TestTheGuardClaimInTheSystemPromptIsTrue` 不许碰）。
...
## 65 — T1222 验收记录：读行函数学会分辨 `!` 行（merged）；我此前记下的「三条待办」里有两条是我读错的（L1 记账 + 立账 T1225，2026-09-24）

### 一、交付与验收

PR #369 于 2026-09-24T09:32:26Z 合并，merge commit `491c65e`。三个改动文件：`tests/security/sast.sh`（+104/−6）、`tests/security/sast-go-surface-check.sh`（+269/−5）、`ops/ci/gosec-baseline.txt`（+16/−5），全部落在 `allowed_scope` 内。

**我的独立 G2**：那台仪器十个案例全绿；基线 256 条条目逐字节相同；被删的 19 行逐条交代过（没有一处是削弱）；工作树按原样留下。

**复核**：`approve`，0 blocking / 0 major / 2 minor / 3 nits / 6 risks，`diff_sha 877809ff…`。复核**没有采信 RESULT.json**：它用仓库里现成的 `sub/.env.example`（被 `.env.*` 匹配、又被 `!.env.example` 重新纳入）自己重推了一遍机制，并且自己验了**丢覆盖那个方向不可能**（被父目录排除压住的重新纳入，git 打的是**排除**那条规则）。红-可红性它自己构造了两处：MUTATION A 只红案例 7/8、MUTATION B 只红案例 9，另有冻结哈希证明改动确实还原了。

### 二、我此前把三条发现记成「待办」——逐条核实后，只有一条成立

复核记了 2 minor + 3 nits，我当时把其中三条抄成了待办，准备折进 T1224 的任务书。**去树里核实之后，两条站不住，而它们各自的严重级复核本来就已经标对了**：

- **「TAB-less 行被静默丢弃，而两个调用点都 `2>&1`，git 的 stderr 就看不见了」（复核标 minor）**：两个调用点**都检查返回码**——文件侧非 0/1 就 `die` 并把 git 的输出原样打出来，目录侧同理。而 rc=1 时 git **一行都不打印**（没有路径被忽略）。所以能被丢掉的只剩「与判决行同时出现的 stderr 噪音」，那本来也不是判决。**撤回。**
- **「`cleanup_plants()` 只清原来三处植入物」（复核标 nit）**：我只读到那一个函数。`cleanup()`（EXIT trap）**清的是全部六处**；`cleanup_plants()` 是**跑前**扫尾那一个，管的正是复核说的场景——**异常退出**留下的残骸，下**一次本地**运行不清它。而案例 10 是**问树**（不是问自己的 `rm`）确认新三处都没了。残留影响因此收窄到「本地异常中断后下一次本地运行可能假红」，CI 每次都是干净检出，够不着。**降级为记录，不立账。**
- **「读 pattern 的方式假定源路径里没有冒号」（复核标 minor）——这条成立**，见下节。

**教训**：复核的严重级就是它的排序，我转抄成待办时**默认把三条都升成了必修**。三条里两条经不起核实，而复核自己其实标得很准（minor / nit）。**先核实再升格**，别把 nit 抄成必修——那既给工人造无谓的活，也让任务书的判据变浑。

### 三、成立的那条 → 立账 T1225

读行函数取 pattern 用的是「剥最短前缀到第一个冒号（当源）、再剥一次（当行号）」。**源路径里有一个冒号就剥错**：我自己构造并量过——`.gitignore` 放在 `dir:with:colon/` 里、内容 `*.go` + `!keep.go`，git 打出 rc=0 与 `dir:with:colon/.gitignore:2:!keep.go<TAB>dir:with:colon/keep.go`，而现行解析出 `colon/.gitignore:2:!keep.go`，**不以 `!` 开头 → 被读成命中**，正是 T1222 存在的理由换个方式回来。

两个方向：文件那一路 → 健康树判红（响）；**目录那一路 → 该目录进 `DROPPED` 而不是 `SURFACE`，扫描面静默缩小**——T1220 修的就是这个类。今天仓库里没有含冒号的目录，所以是潜伏的；而 T1224 正要让这台仪器每轮都跑，潜伏就成了常设。修法是**认行号字段**（纯数字）而不是数冒号。**立 T1225**（`tests/security/**`，依赖 T1222 与 T1224——后者的依赖边同时把两个共用目录的任务串行化）。

### 四、复核自己列的六条风险里，第 1 条与第 3 条正是我刚立的两笔

这不是巧合，也不能含糊过去：

- 它列的**风险 1**：「这台仪器仍然没人跑」——`tests/security/sast-go-surface-check.sh` 不被任何 CI 作业调、也不被总门调。**这正是 T1224**（接成总门第 18 行、`MIN_CHECKS` 17→18）。
- 它列的**风险 3**：解析依赖 `<source>:<linenum>:<pattern>` 这个格式，而**只有否定那一支被测到**；**源路径里的冒号属于未测形态**。**这正是 T1225**。

也就是说：这两笔不是我另起炉灶想出来的，是复核报告里已经点名的两处缺口。**把复核的风险清单当成待办来源，而不是把发现清单当成待办来源**——风险那条线记的才是「这份交付仍不覆盖什么」。

### 五、两处记账教训

1. **「一行 grep 为空」不等于「这句话不存在」**。T1222 的验收判据要求 `grep -rn 'whole Go module'` 为空，它确实为空——但**同一句话有一个折行近亲**活在 `scripts/staticcheck.sh:3-4`（`… run the pinned staticcheck over the whole Go` / `# module and fail on any finding …`），单行 grep 结构上抓不到。判据没错（那在另一个仪器里、也真在范围内），但**「短语不存在」这个结论要按内容找、不能按单行找**。
2. **复核报告里的「392 insertions, 19 deletions」是数了 `+++`/`---` 表头的产物**。逐文件数是 +16/−5、+269/−5、+104/−6，合计 **+389/−16**，与被合并的那笔**逐文件完全一致**。我一度把它当成「复核过的 diff 与合并的 diff 不一样」，查下来是**统计口径**的差别，不是内容的差别。**对不上数时先查口径，再怀疑内容。**

### 六、遗留（记录，不立账）

- 复核为指纹记下的一件事：复核工作树根上有个 5,686,424 字节的构建产物 `api`（被根 `.gitignore:34:/api` 忽略，所以对 `git status`、diff、范围校验都不可见）。**主树里没有这个文件**，而忽略规则保证它进不了提交。不是问题，记一笔。
- 复核的风险 2、5、6 照原样留在它的报告里：新案例依赖 git `-v` 的当前接口（会**假红**而不是假绿）；基线按行号记账，将来在已基线发现上方插行就会红；那两处突变的红证只能靠 `/tmp` 日志加一次已还原的改动复现，交付形态本身没法重跑。

## 66 — `update_progress.py --check` 是一台只能说「不」的仪器，我差点据此把它接进 CI；我自己的自检里也被负控制找出了一个洞（立账 T1228 + 一处引用更正，2026-09-24）

### 一、发现，以及它为什么值得单独立一笔

收尾时顺手核 `tasks/progress.md` 的自动段是否过期——**确实过期了**：表里 T1222 还写着 `todo`，头上写着 `merged 163（合计 164/164 个任务）`、生成时间 `2026-09-24T08:33:07Z`，而当时实际是 **169 个任务 / 164 merged / 2 todo / 1 running / 2 verification**。

顺着去跑 `python3 scripts/update_progress.py --check`，**退出码 1，报「progress view is stale」**。看着像是一道很灵、只是没人接进 CI 的检查——**我差点据此立一笔「把 `--check` 接进 `task-state` 作业」。那会让 main 永远红。**

去读源码，两处就把机制钉死了：

- `render_section`（`scripts/update_progress.py:84-86`）把 `生成时间：{stamp}` 写在**被比较的正文里**；
- `main()`（`:127`）`stamp = args.stamp or now_stamp()`——默认为**当下这一秒**。

沙箱 `--root`，固定戳，**唯一变量就是那个戳**：

```
regen  --stamp 2026-01-01T00:00:00Z        -> rewritten
--check --stamp 2026-01-01T00:00:00Z       -> "progress view up to date"    exit 0
--check --stamp 2026-01-01T00:00:01Z       -> "progress view is stale"      exit 1
--check (不带 --stamp，真实调用)            -> "progress view is stale"      exit 1
```

**对任何已提交的 `progress.md`，真实调用永远返回 1**——除非恰好在同一秒里重生成过。所以**我那次 exit=1 什么也没测出来**：账本确实旧了，但那个退出码不是它的证据。**一台只能说「不」的仪器，和一台只能说「是」的仪器一样没有价值。**（这是我第 65 条那条教训的镜像：那次我把 `nit` 抄成必修，这次我把噪音读成证据。）

### 二、它当初为什么「看起来是好的」

`tasks/results/T0008/RESULT.json` 里 `--check` 的证据是 **rc=0**，原文是 `notice: tasks/progress.md has no auto section yet — run without --check once to seed it`。那时真文件**还没有自动段**，走的是 `:139-142` 的 `current is None → return 0` 早退——**它根本没比较过任何东西**。

自动段一旦种下，这道检查就永远红。也就是说：**一台只在「没东西可比」时绿的检查，在 fixture 里是绿的**（单测总是先种段再比），而在真树上第一次比较就红。这不是 T0008 的过失，是它没法在自己那一刻发现的东西——**但它是「绿过一次不算证据」的又一个实例**。

### 三、我自己的自检里有个洞，是负控制找出来的

按惯例，我对 `mk-T1228.py` 的自检跑了负控制（8 个突变，每个都必须被断言抓住）。**头一轮 `wrong-g3-job` 的退出码是 0——自检漏了。** 原因：我把 `"task-state"` 硬写进了 steps 那行查询，而不是键在 `jobs` 变量上，所以把 `jobs` 指向别的作业时，检查看的仍是 `task-state`。改成一并断言「存在 / 是 required / `requires_tasks ⊆ dependencies` / 它的 steps 真跑那支单测」之后，**8/8 CAUGHT**（另补了 `ghost-g3-job` 一条）。

**记一笔的理由**：这次抓到洞的是负控制，不是复核。**「我写的自检」和「我写的正文」一样需要被证明能失败**——尤其当它守的正是我最在意的那条缝时。

### 四、一处引用更正：我把自己这一笔的出处引错了，而且已经推上去了

立项时我在 T1228 的状态说明里写了「本笔是 `tasks/decisions.md` 第 65 条引出的待办里可以立即执行的一条」。**写进 decision 之前我去树里查了一遍，这句是错的**：第 65 条的「遗留（记录，不立账）」只列了 `api` 构建产物与复核风险 2、5、6；`grep` 全份 `decisions.md`，对 `update_progress` 只有两处历史提及（`:9032` 讲生成器只重写 `AUTO-PROGRESS` 之间、`:13924` 一句泛泛的提醒），**没有这一条**。真实出处是我自己的核对，与第 65 条无关。

更正已按「新鲜读-改-写」落进 `task_status.json`（只换 T1228 的 `notes`，不碰别人的字段），并全文扫过一遍：**别处没有第二处**。**先核引用、再写进 decision**——第 65 条已经教过我一次「转抄要把严重级带上」，这次是「转抄要把出处核上」。

**后果可控是无意的运气**：那句话说在状态文件里，而 Worker 的任务包是 `RenderTaskPackage` 从 DAG 条目渲染的、**不读状态说明**，所以这句话没有进任何工人的提示词。

### 五、立账 T1228

- **范围**：`scripts/update_progress.py` + `scripts/tests/progress-update-test.sh`（两个交付物）。
- **判据核心是「两侧都能说」**：沙箱里「当前 → 退出 0」与「故意弄 stale → 退出 1」**两次都要给原文**，只给一侧不算通过——特别是只有「弄 stale 会红」那一侧（那正是现在唯一会说的一侧）。
- **新用例必须能红**：撤销修法后新用例确实失败，要有原文。既有用例一条不许删、不许跳过、不许放宽。
- **G3 = `task-state`，且不需要新接线**：该作业的第 3 步就是 `bash scripts/tests/progress-update-test.sh`，而它是 required job——改完自动被跑到。（我实测过 `gates.json` 里它那 4 步才开始写书。）
- **为什么 `tasks/**` 对工人是禁区，哪怕要验的就是 `tasks/progress.md`**：那两行是**产物**，开写权限等于允许工人手改产物让检查变绿。所有验证走临时 `--root`，生成器本来就为测试留了这个口子。
- **接线不在本笔范围内，而且顺序不能反**：把 `--check` 变成一道 Gate 要动 `specs/orchestrator/gates.json` 与/或 `.github/workflows/ci.yml`，两者都是 Supervisor-only。**先让仪器两侧都能说，再接**——我在它还会永远报 stale 的时候就差点接上去。任务书明文禁止工人顺手接线。

同笔重生成 `tasks/progress.md`（过期是真的，已修），标记重生成到 `sha256:6d1e869d8b55e4b6`（39 个输入）；`validate_specs` 12/12、`validate_task_state` 9/9、`make check-spec-version` ok。

### 六、遗留（记录，不立账）

- **自动段整个不存在时 `--check` 返回 0**（`:139-142`）：这是**有意的**——消息本身就写着「run without --check once to seed it」，空仓库上不该红。代价是**手删掉自动段的树**会因此报「up to date」。低价值、且改它会动到「还没种」这个正当状态，**不扩进本笔**；记在这里，将来若要收紧只改这一支。
- **接线与否是我的决定**，在本笔合并后自己裁定（要动 CI 配置）。
- 第 65 条的遗留照原样留着：`api` 构建产物（被根 `.gitignore:34:/api` 忽略，主树里没有），以及复核的风险 2、5、6。

### 七、补一条实测：修好的 `--check` 接进去会是什么结果（2026-09-24）

上面说「接线顺序不能反」，那就得先知道**接上去会红在哪儿**。拿**已提交的 blob** 复现了一遍 `--check` 的比较（`git show <rev>:tasks/{tasks,task_status,progress}.md` → 用**同一份已提交的 `task_status.json`** 重生成 `progress.md` → 逐行比，排除 `生成时间：` 那行）：

| 提交 | 提交时 `progress.md` 头一行 | 结果 |
|---|---|---|
| `6dd3f77`（T1226 入账） | `merged 163（合计 164/164 个任务）` | **不一致，8 行** |
| `744189a`（T1227 入账） | `merged 163（合计 164/164 个任务）` | **不一致，9 行** |
| `e31d2a5`（T1228 入账，同笔重生成） | `merged 164（合计 170/170 个任务）` | 一致 |
| `da0b640`（第 66 条，同笔重生成） | `merged 164（合计 170/170 个任务）` | 一致 |
...
## 68 — 驳回 T1223：一句「引号跨行」的机制、一个「同一个语料」的归属——**两处都出在我的任务书里**；外加我自己的提交被自己刚修好的仪器抓到（2026-09-24）

**这一条有两件事：一件是裁定（T1223 驳回返工），一件是自证（我 30358a9 那份进度视图是陈旧的，抓到它的是 T1228 刚修好的那台检查）。两件的共同点是：错都在我这边。**

### 一、裁定：T1223 驳回返工，不是「记录后合并」

`rddev task accept T1223` 在 `2026-09-24T10:39:21Z` **被拒**：`G1 passed · G2 passed · G3 passed · G4 failed`，唯一理由是「最近的评审裁定是 `request_changes`，有 1 条 blocking」。G2 的 11 个作业全绿、G3 的 `mof-canonical` 单步 exit 0、守卫两份副本 `sha256 cc28baae…` **逐字节相同**（我复算过）、套件 **375/375**、`floor` 375。

评审共 5 条：`[0] blocking`（机制说错）、`[1] minor`（README 为一句可测为假的话背书）、`[2] minor`（一个计数没有语料/选择器）、`[3] nit`（失败信息归属过期）、`[4] nit`（一条 ALLOW pin，与既有做法一致，**不派工**）。

**我为什么判「驳回」而不是按 [[reject-vs-record-rule]] 那条「机制说错、防护仍在 ⇒ 记录后合并」：**

- 那条规矩的适用面是**设计选择的解释略有偏差**、而行为已被他人验证正确（T0503 那三轮的教训：散文没有不动点，每轮修一句会引入新的一句）。**本轮不是那个形状。** `[0]` 不是「解释略偏」，是**同一段里第 71 行与第 66 行直接互相矛盾**——第 66 行自己写着「resets its quote state at the start of the next」，第 71 行却说多行那份之所以被拒是「因为引号在第 1 行打开、到最后一行才闭合」。后者若为真，动词就在引号内、**不可能**落在命令位，反而推出「不该拒」。一个读者不可能同时按这两句行动。
- `[1]` 不是机制类，是**「这句是对的，你可以据此停止核对」**那一类——正是那条规矩里「驳回」的一侧。README 新写的那句「The guard's own header states the same boundary, **and stated it correctly before this file did**」，为头注 `:147` 的 `Only an *unknown tool name* exits 0` 背书；而**紧挨它上面三行**，README 自己就列着三条**已知工具名** exit 0 的实测。
- 三处修改都是**减法或一句话**，有不动点，不是散文跑步机。

**裁定：驳回，同 session 返工**（`rddev task reject` → `rddev worker rework --parallel 4`，`2026-09-24T10:42Z` 起）。返工书逐条给了判据与要贴的原文；第一条明写「**你的实现按原样接受**，不要求动任何判定逻辑、不许增删跳过放宽任何用例、`floor` 保持 375」。

### 二、我自己的两处错（这是本笔驳回的真正起因）

**（a）那个计数是我的，而我的任务书把它的一半信息丢了。** 我在 `T1223` 的 `requirements[1]` 里写「我在仓库**全部转录**上跑了一遍……共 **178** 条……**30 条**是新守卫拒、旧守卫放（另有 `93` 条两边都放）」。那个数的出处是**第 63 条 §二.3 的那张表**，而表有**两行、两个语料**：

| 语料 | Bash 调用 | 多行 | 命中 | 误拦 |
|---|---|---|---|---|
| 仅工人会话 | 78,138 | 8,667 | **45** | **4** |
| **全部会话（含我自己的）** | 116,323 | 23,281 | **178** | **30** |

我的书只留了「全部转录」四个字，**既没说「含我自己的会话」，也没把选择器写进去**。工人于是扫了它够得着的语料（`.rddev/workers/*/worker.log` + `worker-run-*.log`，54,129 条去重命令），得到 **23 / 4**，**复现不出 178**，于是把注释写成「A wider pass over **the same corpus** with a looser selector counted 178 such commands and 30 newly refused」——**「the same corpus」是假的**，因为那不是同一个语料。它自己那行其实对应我表里的**「仅工人会话 / 45 / 4」**：同一个现象、**同一个 4**，条数不同只因语料与去重不同。

**规矩写下来**：**数字离了语料和选择器就不能进任务书。** 给不出来就不要这个数——书里点了一个自己说不清出处的数，就是「一个真测量变成一个假句子」的机制，而假的那半（归属）恰是后来读者赖以判断的那半。已记进 `quote-a-measurement-with-its-corpus`。

**（b）窄化范围把一句已知为假的话变成了下一笔的前提。** 第 62/63 条里，T1221 的评审已把 `.rddev.example/README.md:59` 处同一处过宽记成 **nit**；我当时把 T1223 的头部改动**窄化**到 `:19-22` 与 `:51-53`，并叮嘱不要放松那一段。工人**照办了**，同时**把它标记出来而不是擅自改**——**这两件事都是对的**。错的是我：被我特意留下不动的那一行，紧接着被本笔新写的 README 句子**当成正确的引用了**。窄化不是把缺陷停在那里，而是**给它签了字**——「别动这几行」会被读成「这几行不成问题」。已记进 `a-deferred-fix-becomes-the-next-tasks-premise`。

### 三、评审的一处错（我核对过才没有跟着它走）

评审 `[2]` 写这个数「non-reproducible」，「no selector tried from this side lands on 178」。**它试过的选择器没落在 178 上是真的；「不可复现」是不准确的。** 那个数**可复现**——语料（全部会话，116,323 条 Bash 调用、23,281 条多行）与选择器（「每条被守卫点名的行都落在 heredoc 正文或未闭合引号里」）**都写在仓库里**，就在第 63 条 §二.3。它是**出处没进注释**，不是**数不存在**。

**这条很重要：如果我把评审的话照单全收，我就会下令删掉一个真测量。** 所以返工书第 3 条明确写「**不要**因为评审说它不可复现就把它当假数据删干净——它是真的」，给的两条路是「把语料与选择器写进注释／指到第 63 条」或者「删掉 wider pass 那两句、只留你自己可复现的 23/4」。**评审的 citation 也会错**，这一条与 `verify-claims-in-my-own-letters` 记的是同一件事的另一侧。

### 四、工人在这轮里做对的两件事（不许读成过错）

1. **它复现不出我的数，就如实写了——写了四处**（`acceptance[8]`、`risks[0]`、`follow_up_issues[2]`、`notes_for_supervisor`），并且**拒绝把 178/30 当成自己量到的**去写，只把它作为「a wider pass」附带。
2. **它守住了窄化的范围**，没有擅自去改那句过宽的头注，而是**标记出来**。

**但这两件事也暴露了一条要记的：那四处都在 `.rddev/workers/T1223/RESULT.json` 里——不在版本库里，驱动的 accept 一条都不读，我只在评审时读。** 所以「工人说它复现不出我的数」这件事**没有任何东西扛**，它随着本轮一起进了评审，最后以 G4 拒绝、返工一轮收场——而它本来是我写书时一句话就能了结的。**这是 `book-handing-back-creates-an-untracked-obligation` 的第二个实例，极性相反**：不是「接下来交给我」，而是「这里有件只有你能定的事」，停在同一个地方。已补进那条记忆。

### 五、顺带自证：我 `30358a9` 那份进度视图是陈旧的，而抓到它的是刚修好的那台检查

用 T1228 修好的比较方式在**已提交的字节**上量：`30358a9` 的 `tasks/progress.md` 与**同一提交**的 `tasks/task_status.json` 不一致——账本里 T1228 是 `running`（驱动 **10:19:44Z** 写），视图里写的是 `ready`（我 **10:19:43Z** 生成）。**差一秒。** 第 66 条 §七 逐字预言过这个窗口（「跑完生成器到 `git add` 之间驱动写一次」），我一秒宽也照样踩了进去。

**修法就用那条记下的做法**，第一次实跑：先把两份输入冻结进索引（`git add`），再从**索引 blob**（`git show :tasks/…`）而不是活文件生成 `progress.md`，然后 `git add` 它——两份**按构造一致**，与驱动此后还写不写无关。证明用修好的比较方式跑两侧：冻结后的索引 → `progress view up to date` exit 0；在同一份冻结副本里把 `T0001` 由 `merged` 改成 `accepted` → `progress view is stale` exit 1。已提交 `f9a4a6d`（只动 `tasks/progress.md` 一个文件、3 行增 3 行删），已推。

**这就是 T1228 那台仪器的第一个真捕获，而且抓在我身上。** 接线（把 `python3 scripts/update_progress.py --check` 加成 `task-state` 作业的第 5 步）**仍等 T1228 合并**，理由不变：**顺序不能反**——在仪器还不能说「是」的时候接上去，等于让 main 永远红。

**接线本身我已经把形状验完了**（在真文件上应用、跑真 `go test ./internal/devorchestrator/ -run 'TestGatesSpecSyncsWithCIWorkflow'` 绿、再做一次**单边修改的负控制**让 `gateSpecSyncs` 报出 `job "task-state": gates.json has 5 steps, ci.yml has 4`、然后原样撤回，工作树干净）。`gates.json` 必须用 `ensure_ascii=True` 写（它是四份台账里唯一按 `\u` 转义的那份，量过：非 ASCII 字面字节 = 0）；`ci.yml` 只能文本插入（走 YAML 载入会删光注释）；`required_jobs` 仍是 11 个**不变**（这是作业内的第 5 步，不是第 12 个作业），所以证书里那处「11 个作业集合相等」不受影响。

### 六、遗留（记录，不立账）

1. **`[4]` 那条 ALLOW pin**（`tests/worker-guard/guard-regression.sh:640`）：与既有做法一致，不派工。评审提醒它意味着「下一个堵这个洞的任务必须删一条用例并调 `floor`」——记在这里。
