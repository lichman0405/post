...
- **每一步失败都还原** ✓：`reset --hard` / `clean` / `git apply` 三个失败点走同一个 `fail()` ✓ ——
  用 `reset --hard <任务原本的 HEAD>` 恢复任务没改过的部分 ✓，
  再用快照把任务改过的部分**按记录的状态**写回去 ✓（删除的仍然删除 ✓，执行位照旧 ✓）。
...
`rebaseline_test.go` 用真 git 仓库跑三条路径 ✓：拒绝时任务未跟踪的文件 / 符号链接 / 删除 / 执行位**逐项还在** ✓
（用 路径+内容+执行位 的指纹比对 ✓，HEAD 未变 ✓，且错误里必须有那个目录名 ✓）；
成功时活儿**在树上** ✓，且**不留副本** ✓；空改动拒绝时**无副作用** ✓。
mutation battery 7 变体 ✓ **7 caught / 0 missed** ✓，拒绝路径的还原被拿掉后测试立刻变红 ✓。
