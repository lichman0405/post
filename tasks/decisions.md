...
## L1-20260913-20 — ★★ `rebaseline` 的"拒绝"是**破坏性**的：先清空工作区，再说"这份改动不能自动合并"
...
- **被换成目录的那条已跟踪路径，目录里的东西也一起留副本** ✓：
  ...
  **第六轮发现这里原先写得太宽** ✓：原文是"符号链接、空的子目录、fifo 各按自己的种类收" ✓，
  ...
  站在这两类之外的 fifo、socket、空目录 ✓，clean 一样整目录删掉 ✓，副本里却一个条目都没有 ✓
  （见下面"第六轮 review 找出的"一节 ✓）。
  **socket 和字符/块设备收不了** ✓ ——
...
`rebaseline_test.go` **38 个测试 `T9001`–`T9038`** ✓（编号无缺口 ✓，`grep -o 'T90[0-9][0-9]'` 数出来是 38 个互不相同的编号 ✓），
...
- `T9033` 单元级过 `parseCleanLine` ✓：`Would remove` 的两种形式 ✓、`Would skip repository` ✓、
...
- `T9035`（G1 的 socket 版）在一个普通未跟踪目录里放真 socket ✓：拒绝里点明 `sockdir/s is a socket` ✓、
...
**变异电池（`mutate-rebaseline-r6.py`）：44 个变体，44 caught / 0 missed / 0 invalid** ✓，
对照的是本修订的字节 ✓（`rebaseline.go` `sha256:1ea4f324fe5ffdc4…` ✓、
`rebaseline_test.go` `sha256:6ec5f149cc152c10…` ✓、
电池脚本自身 `sha256:521dc4cba473cb5e…` ✓）。每个变体只把一处修复改回原样 ✓
...
第六轮把电池从 33 个变体加到 44 个 ✓，新增的 11 个每个都只钉第六轮的一处 ✓。
...
脚本现在把编译不过计为 INVALID ✓；第三轮 20 个 ✓、第四轮 27 个 ✓、第五轮 33 个 ✓、第六轮 44 个 ✓，
都是加了红基线前置条件之后重跑的 ✓。
...
- **G1（静默丢失，最严重）** ✓：**快照的到达范围是两条 git 清单 ✓，而 clean 的到达范围比它们大** ✓。
  `git status --porcelain` 会把一个整目录未跟踪的目录并成一行 `?? dir/` ✓、里面的东西一个字不提 ✓；
  `ls-files --others --exclude-standard` 只列**文件** ✓ —— 于是 fifo ✓、socket ✓、空目录 ✓、
  以及被 ignore 的文件旁边站着的未跟踪文件 ✓，**两条清单都不列** ✓，而 `git clean -fd` 照样整目录删掉 ✓。
...
`git status --porcelain` 与 `ls-files --others` 都是对"工作区里有什么"的**说明** ✓，
而实际动手的是 `git clean -fd` ✓ —— 两者的到达范围**不一样** ✓，
差额（fifo ✓、socket ✓、空目录 ✓、只有被 ignore 内容的目录 ✓）全部落在"说明"看不见的那一侧 ✓。
这不是第一次同源的错 ✓：`git status --porcelain` 把整目录并成一行这件事 ✓，
在前几轮就已经让"读说明"吃过一次亏 ✓，而这次的结论是**连"再补一条说明"都不对** ✓ ——
补出来的清单等于把挑选标准**写死一次** ✓，它没列出来的仍然是看不见的 ✓，
而"没列出来的"正是差额本身 ✓。正确做法是 `git clean -nd` ✓：
让**同一个操作**在"别动"的模式下自己说 ✓，两条命令行由同一个常量拼出来 ✓，
所以"问的"和"干的"不可能在删什么上不一致 ✓。
