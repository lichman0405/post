...
P2 里没有任何任务依赖它 ✓（它的下游只有 T0402 / T0702）✓。
...
**更糟的是 T0204–T0207 是结构性死锁** ✓，不是"排得早"：
...
2. **把这条不变量写成测试** ✓：`TestEveryG3JobIsSatisfiableByTheTaskThatCarriesIt` ✓，
   ...
   测试点名 9 个 P4 任务并写明"门由构造即红、任务永远无法被接受" ✓。

3. **T0204–T0207 保持不可满足，并被测试显式钉死** ✓：
...
- 注入实验：删一条边 → 测试点名 9 个任务 ✓；恢复 → 全绿 ✓（证明它不空过 ✓）；
