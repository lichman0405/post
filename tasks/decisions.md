...
- **被换成目录的那条已跟踪路径，目录里的东西也一起留副本** ✓：
  ...
  所以路径列表也点不出它的名字 ✓）。
  然后**走文件系统**把里面逐项收进副本 ✓：符号链接、空的子目录、fifo 各按自己的种类收 ✓
  并照原样放回 ✓（fifo 连模式一起 ✓）；**socket 和字符/块设备收不了** ✓ ——
...
`rebaseline_test.go` 26 个测试 `T9001`–`T9026` ✓，真 git 仓库、不 mock ✓。夹具里有：
...
**变异电池（`mutate-rebaseline-r5.py`）：33 个变体，33 caught / 0 missed / 0 invalid** ✓，
对照的是本修订的字节 ✓（`rebaseline.go` `sha256:56176ed063a91ab0…` ✓、
`rebaseline_test.go` `sha256:c4b356510bb79179…` ✓）。每个变体只把一处修复改回原样 ✓
...
脚本现在把编译不过计为 INVALID ✓；第三轮 20 个 ✓、第四轮 27 个 ✓、本轮 33 个 ✓，
都是加了红基线前置条件之后重跑的 ✓。
