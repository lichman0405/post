...
**blocking 与 major 我都独立核实为真** ✓，另有 **3 条 minor 我判断也是真缺陷、一并要求修** ✓ → **已 `rebaseline` 到 `00ac3e2` ✓**，返工中 ✓（迁移 `00034`）；
**T0206**：复核给 **request_changes，一条 blocking** ✓ —— 我独立核实后**成立** ✓（`blob_attachments` 缺 `state_id` ✓），**返工中** ✓（迁移 `00035`）；
**合入顺序被迁移号序钉死** ✓：`T0304`(00032 ✓已合) → `T0603`(00033) → `T0305`(00034) → `T0206`(00035) ✓。
**机制已换成核实过的那个** ✓（我先前引用的 `internal/gooseutil/resolve.go` **从来不存在** ✓，是我编的 ✓，见 `L1-20260914-40` ✓）：
`internal/persistence/migrate.go:42` **没设** `WithAllowOutofOrder` ✓ →
goose v3.28.0 遇到"编号小于库当前最大版本、却未应用"的迁移会**直接报错** ✓（`up.go:77-89` ✓）。
...
最后更新：2026-09-14 12:15（**五件事**：① **`T0603` 已合入 main** ✓（PR #165 → `268fa5e` ✓）——
