...
| #99 | 验收脚本不再改写它正在验收的那棵树（L1-20260913-16）| 已并入 main（`2740e74`）、**7/7 全绿**、**等合入** —— **它挡着 48 个任务**（见下★）|
...
**★★ 而且 #99 挡的不止 T0301 一个 —— 是 48 个任务。** 我刚从 `specs/orchestrator/gates.json`
里数出来 ✓：**G3 里带 `gitea-real-services` 这个 job 的任务有 48 个** ✓
（T0301–T0309、T0401–T0410、T0601–T0609、T0801–T0812、T1201–T1208 ✓），
其中 **46 个还是 todo** ✓、1 个 rejected（T0301 ✓）、1 个 verification（T0603 ✓）、**已合并的 0 个** ✓。
这个 job 跑的就是那个**会改写被测树**的脚本 ✓ —— 也就是说：**#99 不合，这 46 个任务会一个接一个
被同一个理由冤枉掉** ✓（T0301 是第一个 ✓，不是唯一一个 ✓）。这不是"顺手清理" ✓。
