...
**T0603**：复核 approve 带一条 major ✓ → 我核实后**返工** ✓（同一 Worker / 同一 session / 同一基线 ✓）→ **返工已完成、collect 干净** ✓，
且驱动**正确地把那份旧 approve 判为过期** ✓ 并派了新复核 ✓（`T0603-review` 在跑 ✓）——
所以**它随时可以进入 commit → PR → CI → merge** ✓；
**T0206**：复核给 **request_changes，一条 blocking** ✓ —— 我独立核实后**成立** ✓（`blob_attachments` 缺 `state_id` ✓），**返工中** ✓（迁移 `00035`）；
**T0305**：返工已完成 ✓、复核在跑 ✓（迁移 `00034`）✓；
**合入顺序被迁移号序钉死** ✓：`T0304`(00032 ✓已合) → `T0603`(00033) → `T0305`(00034) → `T0206`(00035) ✓。
最后更新：2026-09-14 11:42（**三件事**：① **T0304 合入 main** ✓，`00032` 落地 ✓，
**四个在飞任务的合入顺序由号序唯一确定** ✓（`L1-20260914-31` ✓）；
② **T0603 的旧 approve 被驱动判为过期并自动派了新复核** ✓ —— 陈旧判定这一环是好的 ✓；
③ **我第二次下错判断并被实测推翻** ✓：我说那条 `accept` 决策"无害、会自己化解" ✓，
**实际是它让驱动整个跳过该任务** ✓（`L1-20260914-34` ✓，已补到 #163 ✓））
