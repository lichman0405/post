...
>
> **撞的到底是什么（这是今天最有意思的一处）**：T0213 和 T0209 **都**往 `rsg.Service` 加了一个叫 `profiles`
> 的字段，而它们指**两件事** —— T0209 的是**用户** profile（`GetByUserID`，给对象详情页把 creator id 换成显示名），
> T0213 的是**项目 schema** profile（`GetLatestProfile`，判定 `schema_ref` 是否已注册）。
