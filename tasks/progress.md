...
**T0603**：复核 approve 带一条 major ✓ → 我核实后**返工** ✓ → **新复核给 `approve` ✓，无 blocking、无 major** ✓
（那条错误泄漏确认修掉 ✓）；随即撞上基线问题（`cmd/api/main.go:46` ✓，T0304 也改了这个文件 ✓）→ **已 `rebaseline` 到 `00ac3e2` ✓**，
现在**只在新基线上重验、不允许改代码** ✓（`L1-20260914-36` ✓），返工中 ✓；
...
最后更新：2026-09-14 11:55（**五件事**：① **T0305 与 T0603 都因基线推进被拒** ✓，
