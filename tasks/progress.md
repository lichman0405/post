...
状态：**P0 完成**（14/14）、**P1 完成**（10/10）、**T0201 已合并**；P2 的 T0202 已开工，P3/P6 各卡一处。
最后更新：2026-09-13 23:35（**5 项 orchestrator 变更待人工批准**；自修两条已自行合入：`#108`、`#110`）
...
- **#109 已开** ✓（CI 全绿 ✓，等独立 review 回来再合 ✓）：同一个"门为无关理由变红"的毛病 ✓ ——
  监听器测试把 18981/18982 写死 ✓，而它自己的产物就是一个**逃出会话的监听器** ✓，
  上一次跑漏下的进程正好占着那两个口 ✓。现在两个端口都向内核要 ✓（两个 socket 同时持有 ✓，
  所以两个号必然不同 ✓），断言也从裸数字换成 `/proc/net/tcp` 那个完整地址 ✓（`:1898` 是 `:18981` 的前缀 ✓）。
- P0 / P1 已全部合并 ✓
