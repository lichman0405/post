...
状态：**P0 完成**（14/14）、**P1 完成**（10/10）、**P2 已合 8 个**（T0201 / T0202 / T0203 / T0204 / T0205 / T0215 / T0302 / T0207）；
T0208 在跑 ✓，**T0303 已合入** ✓（PR #158 → `83a2219`）✓，
**T0304 四门全过、已验收** ✓（正被 driver 走 commit→PR→merge）✓，
T0603 复评为 approve ✓、**验收卡在 G3，原因是 T0208 的端点还没合入**（**有证据**的排期，不是猜的）✓。
最后更新：2026-09-14 10:40（**三件事**：① **T0303 合入**、**T0304 验收通过** ✓；
② **我在 Issue #157 上发了一条和正文自相矛盾的结论，已公开更正** ✓（`L1-20260914-21`）✓；
③ **#141 的前提被推翻** ✓：那条「Worker 的 `RESULT.json` 会不脱敏进仓库」的**路已经不存在** ✓
—— 量了 64 条 userinfo，**64/64 用的都是已公开的值** ✓，而 collect **本来就扫** `RESULT.json` ✓。
判据换成「**值在基线树里查不到**才失败」，见 `L1-20260914-22` ✓，代码未动 ✓）
