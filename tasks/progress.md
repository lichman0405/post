...
>
> **两条直接有用的结论**：
> 一，**T0309 此刻不在链上** —— 它的树里没有任何主线没有的迁移。
> 但**这句有保质期**：T0309 还在跑，而 `infra/migrations/**` 在它的范围内（预留号 `00047`），
> 它要是收工前写了迁移，就**当场入链、排在最后一节**（在 T1001 的 `00046` 之后）。
> 这正是 `L1-59` 那条教训：**判据是文件系统，而且要到最后一次 collect 才算数** ——
> 所以我不写"它一定不在链上"，只写"此刻没有"。
