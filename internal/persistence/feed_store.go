...
// canonical queries filter private asset versions out, because the lists are
// bounded and a window read raw is a resource a writer can exhaust (see the
// header of the SQL file). That filter is a read strategy; the RULE that
// decides what may be rendered still belongs to internal/application/feeds
// (BuildFeed), which applies it to every entry whatever the reader returned.
//
