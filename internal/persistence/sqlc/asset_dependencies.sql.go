...
const listProjectAssetUsages = `-- name: ListProjectAssetUsages :many
...
type ListProjectAssetUsagesRow struct {
	Pin               string             `json:"pin"`
...
func (q *Queries) ListProjectAssetUsages(ctx context.Context, projectID pgtype.UUID) ([]ListProjectAssetUsagesRow, error) {
	rows, err := q.db.Query(ctx, listProjectAssetUsages, projectID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListProjectAssetUsagesRow
	for rows.Next() {
		var i ListProjectAssetUsagesRow
		if err := rows.Scan(
			&i.Pin,
			&i.Title,
			&i.AssetType,
...
const recordAssetDependency = `-- name: RecordAssetDependency :exec
...
type RecordAssetDependencyParams struct {
...
func (q *Queries) RecordAssetDependency(ctx context.Context, arg RecordAssetDependencyParams) error {
	_, err := q.db.Exec(ctx, recordAssetDependency,
		arg.ProjectID,
		arg.AssetVersionID,
		arg.DependencyType,
		arg.VisibilityOfUsage,
	)
	return err
}
