...
command -v psql >/dev/null 2>&1 || {
  echo "G3 auth-real-services: FAILED — psql is required to give this run its own database" >&2
  exit 1
}
SCRATCH_DB="post_g3auth_$(date +%s)_$$"
if ! psql "$PG_URL" -q -v ON_ERROR_STOP=1 -c "CREATE DATABASE \"$SCRATCH_DB\"" >/dev/null 2>&1; then
  echo "G3 auth-real-services: FAILED — could not create this run's own database ($SCRATCH_DB) on the admin endpoint (make infra-up && make infra-init)" >&2
  exit 1
fi
...
')
if [[ "${#DB[@]}" -ne 6 ]]; then
  echo "G3 auth-real-services: FAILED — could not read the connection parts out of the admin URL" >&2
  exit 1
fi
