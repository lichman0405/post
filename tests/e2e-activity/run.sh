#!/usr/bin/env bash
# Activity timeline e2e (T0607 required test "activity e2e"): builds the web
# app, serves it with `next start`, and drives the project's Activity tab in
# real Chromium against a network-layer mock of the Go API. The wire shapes
# the mock serves are the ones tests/integration/activity_sources_test.go
# locks against real PostgreSQL (the two registries, the source filter, the
# keyset cursor), so the browser half covers the rendering and the Go half
# covers the read path.
#
# Self-contained like tests/e2e-shell and tests/e2e-settings: the harness
...
#
# Usage: bash tests/e2e-activity/run.sh
# Env:   ACTIVITY_E2E_PORT (default 31110) overrides the server port.
set -euo pipefail

E2E_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT="$(cd "$E2E_DIR/../.." && pwd)"
WEB_DIR="$ROOT/apps/web"
PORT="${ACTIVITY_E2E_PORT:-31110}"
...
if ss -tln 2>/dev/null | grep -qE ":$PORT[[:space:]]"; then
  echo "e2e-activity: port $PORT is already in use; a previous run may have leaked a server" >&2
  exit 1
fi

echo "== e2e-activity: installing harness deps (own lockfile, npm) =="
(
  cd "$E2E_DIR"
...
echo "== e2e-activity: browser library bootstrap (shared web-smoke helper) =="
LIB_PATH="$(bash "$ROOT/tests/web-smoke/bootstrap-deps.sh")"
export LD_LIBRARY_PATH="${LIB_PATH:+$LIB_PATH:}${LD_LIBRARY_PATH:-}"
export PLAYWRIGHT_SKIP_VALIDATE_HOST_REQUIREMENTS=1

echo "== e2e-activity: building the web app (API origin baked in) =="
(
  cd "$WEB_DIR"
  pnpm run build
)

echo "== e2e-activity: starting next on $BASE =="
(
  cd "$WEB_DIR"
...
if [ "$ready" != 1 ]; then
  echo "e2e-activity: server did not become ready; log:" >&2
  cat "$E2E_DIR/server.log" >&2
  exit 1
fi
echo "== e2e-activity: server ready =="

echo "== e2e-activity: the Activity timeline checklist =="
if ! node "$E2E_DIR/activity-e2e.mjs" "$BASE" "$WEB_DIR"; then
  echo "e2e-activity: FAILED (server log: $E2E_DIR/server.log)" >&2
  exit 1
fi
echo "e2e-activity: all passed"
