#!/usr/bin/env bash
# Project settings e2e (T0109 required test "settings e2e"): builds the
# web app, serves it with `next start`, and drives the settings checklist
# in real Chromium against a network-layer mock of the Go API (the exact
# wire shapes cmd/api/projectshttp produces — the integration suite locks
# those shapes against real PostgreSQL). Self-contained like tests/e2e-shell:
...
#
# Usage: bash tests/e2e-settings/run.sh
# Env:   SETTINGS_E2E_PORT (default 31109) overrides the server port.
set -euo pipefail

E2E_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT="$(cd "$E2E_DIR/../.." && pwd)"
WEB_DIR="$ROOT/apps/web"
PORT="${SETTINGS_E2E_PORT:-31109}"
...
if ss -tln 2>/dev/null | grep -qE ":$PORT[[:space:]]"; then
  echo "e2e-settings: port $PORT is already in use; a previous run may have leaked a server" >&2
  exit 1
fi

echo "== e2e-settings: installing harness deps (own lockfile, npm) =="
(
  cd "$E2E_DIR"
...
echo "== e2e-settings: browser library bootstrap (shared web-smoke helper) =="
LIB_PATH="$(bash "$ROOT/tests/web-smoke/bootstrap-deps.sh")"
export LD_LIBRARY_PATH="${LIB_PATH:+$LIB_PATH:}${LD_LIBRARY_PATH:-}"
export PLAYWRIGHT_SKIP_VALIDATE_HOST_REQUIREMENTS=1

echo "== e2e-settings: building the web app (API origin baked in) =="
(
  cd "$WEB_DIR"
  pnpm run build
)

echo "== e2e-settings: starting next on $BASE =="
(
  cd "$WEB_DIR"
...
if [ "$ready" != 1 ]; then
  echo "e2e-settings: server did not become ready; log:" >&2
  cat "$E2E_DIR/server.log" >&2
  exit 1
fi
echo "== e2e-settings: server ready =="

echo "== e2e-settings: project settings checklist =="
if ! node "$E2E_DIR/settings-e2e.mjs" "$BASE" "$WEB_DIR"; then
  echo "e2e-settings: FAILED (server log: $E2E_DIR/server.log)" >&2
  exit 1
fi
echo "e2e-settings: all passed"
