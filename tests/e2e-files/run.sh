#!/usr/bin/env bash
# Files read-only e2e (T0308 required test "files read-only e2e"): builds
# the web app, serves it with `next start`, and drives the read-only Files
# checklist in real Chromium against a network-layer mock of the Go API
# (the exact wire shapes cmd/api/fileshttp produces — the Go handler suite
# locks those shapes against the real guard). Self-contained like
...
#
# Usage: bash tests/e2e-files/run.sh
# Env:   FILES_E2E_PORT (default 31109) overrides the server port.
set -euo pipefail

E2E_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT="$(cd "$E2E_DIR/../.." && pwd)"
WEB_DIR="$ROOT/apps/web"
PORT="${FILES_E2E_PORT:-31109}"
...
if ss -tln 2>/dev/null | grep -qE ":$PORT[[:space:]]"; then
  echo "e2e-files: port $PORT is already in use; a previous run may have leaked a server" >&2
  exit 1
fi

echo "== e2e-files: installing harness deps (own lockfile, npm) =="
(
  cd "$E2E_DIR"
...
echo "== e2e-files: browser library bootstrap (shared web-smoke helper) =="
LIB_PATH="$(bash "$ROOT/tests/web-smoke/bootstrap-deps.sh")"
export LD_LIBRARY_PATH="${LIB_PATH:+$LIB_PATH:}${LD_LIBRARY_PATH:-}"
export PLAYWRIGHT_SKIP_VALIDATE_HOST_REQUIREMENTS=1

echo "== e2e-files: building the web app (API origin baked in) =="
(
  cd "$WEB_DIR"
  pnpm run build
)

echo "== e2e-files: starting next on $BASE =="
(
  cd "$WEB_DIR"
...
if [ "$ready" != 1 ]; then
  echo "e2e-files: server did not become ready; log:" >&2
  cat "$E2E_DIR/server.log" >&2
  exit 1
fi
echo "== e2e-files: server ready =="

echo "== e2e-files: read-only Files checklist =="
if ! node "$E2E_DIR/files-e2e.mjs" "$BASE" "$WEB_DIR"; then
  echo "e2e-files: FAILED (server log: $E2E_DIR/server.log)" >&2
  exit 1
fi
echo "e2e-files: all passed"
